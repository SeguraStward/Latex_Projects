...
class S06_Arquitecturas(Scene):
    """
    Arquitecturas clave en diagnostico medico:
    AlexNet  — clasificacion (hay tumor o no?)
    U-Net    — segmentacion pixel a pixel
    ...
    def _build_alexnet(self, origin):
        """Cadena lineal de bloques: imagen → conv → pool → FC → clase."""
        blocks_data = [
    ...
    def _build_unet(self, origin):
        """
        Encoder baja (izq→centro), decoder sube (centro→der),
        skip connections horizontales.
        """
        enc_data = [
            ("64",  1.4, 0.35, TEAL_C),
            ("128", 1.1, 0.35, TEAL_C),
        ...
        # Posicionar encoder (columna izquierda, bajando)
        x_enc = -2.0
        ...
        # Bottleneck (centro abajo)
        bottleneck.move_to(origin + DOWN * 1.7)

        # Posicionar decoder (columna derecha, subiendo)
        x_dec = 2.0
        ...
        # Skip connections (encoder → decoder mismo nivel)
    ...
    def _build_resnet_block(self, origin):
        ...
        # Conexion residual (skip): arco por encima
        ...
        skip_lbl = Text("x (identidad)", font_size=11, color=YELLOW)
    ...
    def construct(self):
        ...
        # AlexNet
        # ══════════════════════════════════════════════════════════════════════
        title = Text("Arquitecturas Clave en Diagnóstico Médico",
                     font_size=32, color=BLUE_B, weight=BOLD)
        title.to_edge(UP, buff=0.35)
        self.play(Write(title), run_time=0.8)

        arch_lbl = Text("AlexNet — Clasificación",
        ...
        img_lbl = Text("Rx\ntórax", font_size=9, color=BLUE_C)
        ...
        # Salida (derecha)
        out_lbl = Text("Tumor: SI/NO\n(clasificación)", font_size=11, color=GREEN)
        ...
        metric_alex = Text("90.2% precisión en clasificación de pulmón",
        ...
        # U-Net
        # ══════════════════════════════════════════════════════════════════════
        arch_lbl2 = Text("U-Net — Segmentación", font_size=24,
        ...
        # Skip connections en amarillo
        skip_lbl = Text("skip connections\n(recuperan detalle espacial)",
        ...
        # Imagen entrada / salida segmentada
        ...
        # contorno del corazon simulado dentro
        ...
        metric_unet = Text("Predicción pixel a pixel — Alto coeficiente Dice en segmentación cardíaca",
                           font_size=13, color=YELLOW)
        ...
        # ResNet
        # ══════════════════════════════════════════════════════════════════════
        arch_lbl3 = Text("ResNet — Conexiones Residuales",
        ...
        # Skip en amarillo (la clave de ResNet)
        self.play(Create(skip_path), FadeIn(skip_lbl_r), run_time=0.7)
        self.play(Write(formula), run_time=0.7)

        # Problema que resuelve
        problem = VGroup(
            Text("Sin skip:  gradiente desaparece en redes profundas",
                 font_size=13, color=RED),
            Text("Con skip:  redes de 50-150+ capas sin degradación",
        ...
        metric_res = Text("AUC 93.2% en clasificación de gliomas",
        ...
        table_title = Text("Comparativa", font_size=26, color=BLUE_B, weight=BOLD)
        table_title.next_to(title, DOWN, buff=0.2)
        self.play(FadeIn(table_title))

        rows = [
            ("Arquitectura", "Tarea principal",    "Clave tecnica",          GRAY_A),
            ("AlexNet",      "Clasificación",       "Capas conv. + FC",       BLUE_C),
            ("U-Net",        "Segmentación",        "Encoder/Decoder + skip", TEAL_C),
            ("ResNet",       "Alta precisión",      "Conexiones residuales",  PURPLE_B),
        ]
        col_x = [-3.0, -0.2, 2.6]
        table_group = VGroup()
